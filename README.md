...
- `--env` - override ENV analyzing image [can use this flag multiple times]
- `--workdir` - override WORKDIR analyzing image
- `--network` - override default container network settings analyzing image
- `--expose` - use additional EXPOSE instructions analyzing image [can use this flag multiple times]
- `--link` - add link to another container analyzing image [can use this flag multiple times]
- `--hostname` - override default container hostname analyzing image
- `--etc-hosts-map` - add a host to IP mapping to /etc/hosts analyzing image [can use this flag multiple times]
- `--container-dns` - add a dns server analyzing image [can use this flag multiple times]
- `--container-dns-search` - add a dns search domain for unqualified hostnames analyzing image [can use this flag multiple times]
